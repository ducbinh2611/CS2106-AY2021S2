 ...
 *********************************************************/
{
    ...
    partInfo* prevPartition = currentPartition;
    ...
    if (foundBuddy) {
        ...
        // if there's some block at that level already
        // if there exists some free partition at that level already
        if (prev != NULL) {
...
{   
    if (hmi.A[(int)lvl] != NULL) {
        ...
        // find the smallest R from S + 1 to K such that A[R] has a free block
        // now need to split from R - 1 to S
 ...
 *********************************************************/
{
    ...
    if (initialSize < minSize) {
        ...
        if (maxSize < initialSize) {
            largestMemChunk = maxSize;
        } else {
            largestMemChunk = initialSize;
        }
        int maxIdx = (int) log2Floor((unsigned int)largestMemChunk);       
        hmi.A = (partInfo**) malloc(sizeof(partInfo*) * (maxIdx + 1));   //change this!
        hmi.maxIdx = maxIdx; //change this!
        hmi.minIdx = log2Ceiling(minSize);
        for (int i = 0; i <= maxIdx; i++) {
            hmi.A[i] = NULL;
        }
        int remainingSize = initialSize;
        int currAddr = 0;
        int currLevel = maxIdx;
        while (remainingSize >= minSize) {
            partInfo* initialFreePartition = buildPartitionInfo(currAddr);
            if (hmi.A[currLevel] == NULL) {
                hmi.A[currLevel] = initialFreePartition;
            } else {
                partInfo* currPartition = hmi.A[currLevel];
                // traverse to the end of the list to append the new initial free partition
                while (currPartition->nextPart != NULL) {
                    currPartition = currPartition->nextPart;
                }
                currPartition->nextPart = initialFreePartition;
            }
            currAddr += powOf2(currLevel);
            hmi.totalSize += powOf2(currLevel);
            remainingSize -= powOf2(currLevel);
            int nextLevelSize;
            if (maxSize < remainingSize) {
                nextLevelSize = maxSize;
            } else {
                nextLevelSize = remainingSize;
            }
            currLevel = log2Floor(nextLevelSize);
        }
        return 1;
    }
}
 ...
 *********************************************************/
{
    ...
    unsigned int log2 = log2Ceiling((unsigned int) size);
    unsigned int level;
    if (hmi.minIdx > log2) {
        level = hmi.minIdx;
    } else {
    ...
    if (blockAllocated != NULL) {
        hmi.internalFragTotal+= powOf2(level) - size;

 ...
 *********************************************************/
{
    ...
    int numOfOccupiedPartitions = 0;
    int sizeOfOccupiedPartitions = 0;
    int numOfHoles = 0;
    int sizeOfHoles = 0;
    partInfo *ptr = hmi.pListHead;
    while (ptr != NULL) {
        if (ptr->status == OCCUPIED) {
            numOfOccupiedPartitions++;
            sizeOfOccupiedPartitions += ptr->size;
        } else {
            numOfHoles++;
            sizeOfHoles += ptr->size;
        }
        ptr = ptr->nextPart; 
    }
    
    printf("Total Occupied Partitions: %d\n", numOfOccupiedPartitions);
    printf("\tTotal Occupied Size: %d bytes\n", sizeOfOccupiedPartitions);
 ...
 *********************************************************/
{
    ...
    if (bestSoFar == NULL){	//heap full
		return NULL;
	}

	//Do we need to split the partition?
	if (bestSoFar->size > size) {
		splitPart(bestSoFar, size);
	}

	bestSoFar->status = OCCUPIED;
	
	return (void*)hmi.base + bestSoFar->offset;
}
